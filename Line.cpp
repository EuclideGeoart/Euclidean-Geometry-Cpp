 ...
 */
Line::Line(std::shared_ptr<Point> startPoint, std::shared_ptr<Point> endPoint, bool isSegment, const sf::Color& color)
  ...
  if (localDebug) {
    std::cout << "Line Constructor: Creating " << (isSegment ? "segment" : "infinite line") << " with color (" << static_cast<int>(color.r) << ", "
  ...
  try {
    ...
    if (localDebug) {
      std::cout << "Line Constructor: Canonical length = " << CGAL::to_double(m_canonicalLength) << std::endl;
    }
  } catch (const std::exception& e) {
    std::cerr << "Line Constructor: Exception calculating canonical length: " << e.what() << ". Using default." << std::endl;
    m_canonicalLength = Kernel::FT(Constants::DEFAULT_LINE_CONSTRUCTION_LENGTH);
  }
  ...
  try {
    ...
    if (localDebug) {
      std::cout << "Line Constructor: Successfully created line between P" << m_startPoint->getID() << " and P" << m_endPoint->getID() << std::endl;
    }
  } catch (const std::exception& e) {
...
// ------ctr cgal with ID------
Line::Line(std::shared_ptr<Point> start, std::shared_ptr<Point> end, bool isSegment, const sf::Color& color, unsigned int id)
  ...
  try {
    ...
    std::cout << "Line/Segment created with ID: " << m_id << " between P" << m_startPoint->getID() << " and P" << m_endPoint->getID() << std::endl;
  } catch (const std::exception& e) {
    std::cerr << "Line Constructor (ID variant): Exception during initialization: " << e.what() << std::endl;
    throw;
  }
}
...
Line::~Line() {
  ...
  try {
    ...
    if (!m_hostedObjectPoints.empty()) {
      std::cout << "Line destructor: Clearing " << m_hostedObjectPoints.size() << " hosted ObjectPoints" << std::endl;
...
void Line::calculateCanonicalLength() {
  ...
  if (CGAL::abs(m_canonicalLength) < Kernel::FT(Constants::EPSILON_LENGTH_CONSTRUCTION)) {
    if (Constants::DEBUG_LINE_CREATION) {
      std::cout << "Line Constructor: Calculated length too small (" << CGAL::to_double(m_canonicalLength)
...
void Line::registerWithEndpoints() {
  ...
  if (m_startPoint) {
    try {
      ...
      m_startPoint->addConnectedLine(shared_from_this());
      std::cout << "Successfully registered line " << getID() << " with start point " << m_startPoint->getID() << std::endl;
    } catch (const std::exception& e) {
...
void Line::setExternallyMovedEndpoint(Point* movedPoint) {
  ...
  if (!m_constraintObservers.empty() && !m_inConstraint) {
    if (Constants::DEBUG_CONSTRAINTS) {
      if (Constants::DEBUG_CONSTRAINTS) {
        std::cout << "Line " << getID() << " notified that Point " << movedPoint << " was externally moved" << std::endl;
      }
...
void Line::updateCGALLine() {
  try {
    ...
    try {
      ...
      std::cerr << "Line::updateCGALLine: Error getting endpoint positions: " << e.what() << std::endl;
      return;
    }
    ...
    try {
      ...
      std::cerr << "Line::updateCGALLine: Cannot convert coordinates to double: " << e.what() << std::endl;
      return;
    }
    ...
    if (!std::isfinite(dist_sq) || dist_sq < 1e-20) {
      std::cerr << "Line::updateCGALLine: Degenerate line (distance² = " << dist_sq << ")" << std::endl;
      return;
    }
...
Point_2 sfmlToCGAL(const sf::Vector2f& sfmlPoint) {
  if (std::isnan(sfmlPoint.x) || std::isinf(sfmlPoint.x) || std::isnan(sfmlPoint.y) || std::isinf(sfmlPoint.y)) {
    std::cerr << "Error: sfmlToCGAL called with NaN/Inf coordinates: (" << sfmlPoint.x << ", " << sfmlPoint.y << ")" << std::endl;
...
// robust line.
const Kernel::FT MIN_SQ_LENGTH_THRESHOLD(1e-6);  // Adjust as needed based on typical coordinate scale
...
void Line::draw(sf::RenderWindow& window, float scale, bool forceVisible) const {
  try {
    ...
    if (!m_visible && forceVisible) drawColor.a = 50;

    // --- PIXEL THICKNESS ---
    // Use the object's stored thickness (adjustable via GUI)
    float pixelThickness = m_thickness;
    //if (pixelThickness < 1.0f) pixelThickness = 1.0f;
    // Add visual feedback to the base thickness
    if (m_selected) {
        pixelThickness += 2.0f; 
    } else if (m_hovered) {
        pixelThickness += 1.0f;
    ...
    // We strictly use Integers for thickness to ensure sharpness
    // Convert thickness to world units
    // Ensure it is an integer (e.g., 2.0, 3.0) not 2.1
    pixelThickness = std::round(pixelThickness);

    sf::Vector2f worldP1, worldP2;
    ObjectType type = getType();  // Line, LineSegment, Ray, Vector

    if (m_isSegment) {
      if (m_sfmlShape.getVertexCount() < 2) return;
      worldP1 = m_sfmlShape[0].position;
      worldP2 = m_sfmlShape[1].position;
    } else {
      // For infinite lines/rays/vectors, use the defining points
      // --- COORDINATE CALCULATION ---
      const Point_2 startCgal = m_startPoint->getCGALPosition();
      const Point_2 endCgal = m_endPoint->getCGALPosition();
      sf::Vector2f p1(static_cast<float>(CGAL::to_double(startCgal.x())), static_cast<float>(CGAL::to_double(startCgal.y())));
      sf::Vector2f p2(static_cast<float>(CGAL::to_double(endCgal.x())), static_cast<float>(CGAL::to_double(endCgal.y())));
      ...
      if (len > 1e-9f) {
        ...
        // Extension length: enough to cover the screen diagonally
        float extension = viewDiagonal * 1.5f;

        // --- TYPE-SPECIFIC DRAWING LOGIC ---
        // (Default assignment to avoid uninitialized variable warnings, though logic below handles cases)
        worldP1 = p1;
        worldP2 = p2;

        if (m_lineType == LineType::Infinite || type == ObjectType::Line || m_lineType == LineType::Ray || type == ObjectType::Ray) {
          // RE-CENTERING TRICK:
          // Find the point on the line closest to the center of the view.
          sf::Vector2f viewCenter = currentView.getCenter();
          sf::Vector2f toCenter = viewCenter - p1;

          // Project "vector to center" onto the line direction
          float proj = toCenter.x * dir.x + toCenter.y * dir.y;

          // 'mid' is the anchor point on the line closest to the camera
          sf::Vector2f mid = p1 + dir * proj;

          if (type == ObjectType::Ray || m_lineType == LineType::Ray) {
            // RAY LOGIC: Start at original P1, extend P2 through screen
            worldP1 = p1;
            worldP2 = mid + dir * extension;
          } else {
            // INFINITE LINE LOGIC: Extend both ways from the view center
            worldP1 = mid - dir * extension;
            worldP2 = mid + dir * extension;
          }
        } else {
          // Fallback for degenerates or logic mismatch
          return;
        }
      } else {
        return;  // Degenerate line (length 0)
      }
    }

    // 5. Convert to Screen Space (The Sharpness Trick)
    // We map world coordinates to integer screen pixels.
    // This removes sub-pixel anti-aliasing blur for straight lines.
    sf::Vector2i screenP1 = window.mapCoordsToPixel(worldP1);
    sf::Vector2i screenP2 = window.mapCoordsToPixel(worldP2);

    sf::Vector2f dirScreen = sf::Vector2f(screenP2 - screenP1);
    float lenScreen = std::sqrt(dirScreen.x * dirScreen.x + dirScreen.y * dirScreen.y);

    // Don't draw if it's too small to see on screen
    if (lenScreen < 0.5f) return;

    sf::Vector2f unitDirScreen = dirScreen / lenScreen;
    sf::Vector2f normalScreen(-unitDirScreen.y, unitDirScreen.x);

    // 6. Calculate Offsets in Pixels
    // This creates a quad exactly 'pixelThickness' wide.
    sf::Vector2f offsetScreen = normalScreen * (pixelThickness * 0.5f);

    // Build the 4 corners of the line in Screen Space
    sf::Vector2f v1Screen = sf::Vector2f(screenP1) + offsetScreen;
    sf::Vector2f v2Screen = sf::Vector2f(screenP2) + offsetScreen;
    ...
    if (type == ObjectType::Vector) {
      // Fixed pixel size for consistent arrow look regardless of zoom
      float arrowLenPx = 15.0f;
      float arrowWidthPx = 10.0f;  // Width of the arrow base

      // Use screen coordinates for the tip (P2)
      sf::Vector2f tipScreen = sf::Vector2f(screenP2);

      // Calculate base of arrow in screen pixels
      sf::Vector2f baseScreen = tipScreen - unitDirScreen * arrowLenPx;
      sf::Vector2f leftScreen = baseScreen + normalScreen * (arrowWidthPx * 0.5f);
      sf::Vector2f rightScreen = baseScreen - normalScreen * (arrowWidthPx * 0.5f);

      sf::VertexArray arrow(sf::Triangles, 3);
      // Map back to World to draw
      arrow[0].position = window.mapPixelToCoords(sf::Vector2i(tipScreen));
      arrow[1].position = window.mapPixelToCoords(sf::Vector2i(leftScreen));
      arrow[2].position = window.mapPixelToCoords(sf::Vector2i(rightScreen));

      for (int i = 0; i < 3; ++i) arrow[i].color = drawColor;
      window.draw(arrow);
    }

    // --- 7.5 DRAW DECORATIONS (Sharp / Screen Space) ---
    if (m_decoration != DecorationType::None) {
      // Use Screen Coordinates converted to float for precision
      sf::Vector2f p1Px = sf::Vector2f(screenP1);
      sf::Vector2f p2Px = sf::Vector2f(screenP2);

      sf::Vector2f dirPx = p2Px - p1Px;
      float lenPx = std::sqrt(dirPx.x * dirPx.x + dirPx.y * dirPx.y);

      // Only draw if line is long enough on screen (e.g. 20 pixels)
      if (lenPx > 20.0f) {
        sf::Vector2f centerPx = (p1Px + p2Px) * 0.5f;
        sf::Vector2f u = dirPx / lenPx;
        sf::Vector2f n(-u.y, u.x);

        // Define sizes in PIXELS (Invariant to Zoom)
        float size = 12.0f;
        float spacing = 6.0f;

        // Helper: Draw Tick (Calculate in Pixels -> Map to World)
        auto drawTick = [&](sf::Vector2f posPx) {
          sf::VertexArray tick(sf::Lines, 2);
          // Calculate pixel positions
          sf::Vector2f t1 = posPx + n * (size * 0.5f);
          sf::Vector2f t2 = posPx - n * (size * 0.5f);

          // Map to World for drawing
          tick[0].position = window.mapPixelToCoords(sf::Vector2i(t1));
          tick[1].position = window.mapPixelToCoords(sf::Vector2i(t2));
          tick[0].color = drawColor;
          tick[1].color = drawColor;
          window.draw(tick);
        };

        // Helper: Draw Arrow (Calculate in Pixels -> Map to World)
        auto drawArrow = [&](sf::Vector2f posPx) {
          sf::VertexArray arrow(sf::LineStrip, 3);
          sf::Vector2f tip = posPx + u * (size * 0.5f);
          sf::Vector2f base = posPx - u * (size * 0.5f);

          sf::Vector2f v1 = base + n * (size * 0.4f);
          sf::Vector2f v2 = tip;
          sf::Vector2f v3 = base - n * (size * 0.4f);

          // Map to World
          arrow[0].position = window.mapPixelToCoords(sf::Vector2i(v1));
          arrow[1].position = window.mapPixelToCoords(sf::Vector2i(v2));
          arrow[2].position = window.mapPixelToCoords(sf::Vector2i(v3));
          for (int i = 0; i < 3; ++i) arrow[i].color = drawColor;
          window.draw(arrow);
        };

        switch (m_decoration) {
          case DecorationType::Tick1:
            drawTick(centerPx);
            break;
          case DecorationType::Tick2:
            drawTick(centerPx - u * spacing * 0.5f);
            drawTick(centerPx + u * spacing * 0.5f);
            break;
          case DecorationType::Tick3:
            drawTick(centerPx);
            drawTick(centerPx - u * spacing);
            drawTick(centerPx + u * spacing);
            break;
          case DecorationType::Arrow1:
            drawArrow(centerPx);
            break;
          case DecorationType::Arrow2:
            drawArrow(centerPx - u * spacing * 0.7f);
            drawArrow(centerPx + u * spacing * 0.7f);
            break;
          case DecorationType::Arrow3:
            drawArrow(centerPx);
            drawArrow(centerPx - u * spacing * 1.2f);
            drawArrow(centerPx + u * spacing * 1.2f);
            break;
          default:
            break;
...
bool Line::contains(const sf::Vector2f& worldPos_sfml, float tolerance) const {
  ...
  try {
    ...
    std::cerr << "Line::contains: Exception getting SFML endpoint positions: " << e.what() << std::endl;
    return false;
  }
  ...
  if (lineSqLength < (Constants::EPSILON * Constants::EPSILON)) {
    float distSqToP1 = (worldPos_sfml.x - p1_sfml.x) * (worldPos_sfml.x - p1_sfml.x) + (worldPos_sfml.y - p1_sfml.y) * (worldPos_sfml.y - p1_sfml.y);
  ...
  if (m_isSegment) {
    ...
    float t_numerator = (worldPos_sfml.x - p1_sfml.x) * dx_line + (worldPos_sfml.y - p1_sfml.y) * dy_line;
    ...
    if (t < 0.0f) {  // Closest point on segment is p1_sfml
      distSqToClosest = (worldPos_sfml.x - p1_sfml.x) * (worldPos_sfml.x - p1_sfml.x) + (worldPos_sfml.y - p1_sfml.y) * (worldPos_sfml.y - p1_sfml.y);
    } else if (t > 1.0f) {  // Closest point on segment is p2_sfml
      distSqToClosest = (worldPos_sfml.x - p2_sfml.x) * (worldPos_sfml.x - p2_sfml.x) + (worldPos_sfml.y - p2_sfml.y) * (worldPos_sfml.y - p2_sfml.y);
      ...
      float dist_num_abs = std::abs(dy_line * worldPos_sfml.x - dx_line * worldPos_sfml.y + (p2_sfml.x * p1_sfml.y - p1_sfml.x * p2_sfml.y));
...
sf::FloatRect Line::getGlobalBounds() const {
  ...
  try {
    ...
    std::cerr << "Line::getGlobalBounds: Error getting SFML positions from points: " << e.what() << std::endl;
  ...
  // minimums
  if (width < Constants::LINE_INTERACTION_RADIUS * 2 && height < Constants::LINE_INTERACTION_RADIUS * 2 &&
      (std::abs(startPos_sfml.x - endPos_sfml.x) < Constants::EPSILON && std::abs(startPos_sfml.y - endPos_sfml.y) < Constants::EPSILON)) {
    left = startPos_sfml.x - Constants::LINE_INTERACTION_RADIUS;
    top = startPos_sfml.y - Constants::LINE_INTERACTION_RADIUS;
    width = Constants::LINE_INTERACTION_RADIUS * 2;
    height = Constants::LINE_INTERACTION_RADIUS * 2;
...
void Line::setIsUnderDirectManipulation(bool isManipulated) {
  ...
  if (Constants::DEBUG_DRAGGING) {  // Or your preferred debug flag
    std::cout << "Line " << this << ": m_isUnderDirectManipulation set to " << (isManipulated ? "true" : "false") << std::endl;
  }
}
...
void Line::update() {
  ...
  try {
    ...
    if ((m_isParallelLine || m_isPerpendicularLine) && !m_isUnderDirectManipulation) {
      ...
      if (has_ref_obj || (m_constraintDirection != Vector_2(0, 0) && m_constraintRefObject.expired())) {
        if (Constants::DEBUG_CONSTRAINTS) {
          ...
          std::cout << "Line::update: Calling maintainConstraints for Line " << getID() << " (Ref: " << refId << ")" << std::endl;
        }
    ...
    std::cerr << "Line::update: Exception during update cycle: " << e.what() << " Line: " << getID() << " (" << this << ")" << std::endl;
  } catch (...) {
    std::cerr << "Line::update: Unknown exception during update cycle. Line: " << getID() << " (" << this << ")" << std::endl;
  }
...
void Line::forceConstraintUpdate() {
  if ((m_isParallelLine || m_isPerpendicularLine) && !m_inConstraint) {
    ...
    if (geometryChanged) {
      ...
      std::cout << "Line::forceConstraintUpdate: NO CHANGE - skipping visual updates for Line " << getID() << std::endl;
    }
...
void Line::notifyConstraintObserversOfChange() {
  if (Constants::DEBUG_CONSTRAINTS) {
    std::cout << "Line::notifyConstraintObserversOfChange: Notifying " << m_constraintObservers.size() << " observers from Line " << getID()
              << std::endl;
  }
...
void Line::translate(const Vector_2& offset) {
  ...
  try {
    ...
    std::cerr << "Exception in Line::translate during endpoint setting: " << e.what() << " Line: " << this << std::endl;
  ...
  if (!m_isUpdatingInternally) {
    ...
    if (Constants::DEBUG_CONSTRAINTS && !m_constraintObservers.empty()) {
      std::cout << "Line::translate: Notifying " << m_constraintObservers.size() << " observers post-translation. Line: " << this << std::endl;
    }
    notifyConstraintObserversOfChange();
  }
...
void Line::setAsConstructionLine() { m_isConstructionLine = true; }
...
void Line::translateWithDependents(const Vector_2& offset) {
  ...
  try {
    ...
    // try-catch
    if (!CGAL::is_finite(offset.x()) || !CGAL::is_finite(offset.y())) {  // This is line 1161 from your context
  ...
  if (m_startPoint && m_endPoint) {
    ...
    try {
      ...
      debug_cgal_point(sp_before_loop, "m_startPoint (before loop)", "Line::translateWithDependents");
  ...
  if (!m_hostedObjectPoints.empty()) {
    std::cerr << "Line::translateWithDependents: Processing " << m_hostedObjectPoints.size() << " hosted points." << std::endl;
    for (const auto& objPoint_weak : m_hostedObjectPoints) {
      if (auto objPoint = objPoint_weak.lock()) {
        try {
          ...
          std::cerr << "Line::translateWithDependents: Stored relativeParam: " << relativeParam << " for ObjectPoint " << objPoint.get() << std::endl;
        } catch (const std::exception& e) {
  ...
  try {
    ...
    if (m_isParallelLine) {  // Implies axis-parallel if m_referenceLine is
      ...
      try {
        ...
        std::cerr << "Line::moveEndpointToStart: Error getting positions: " << e.what() << std::endl;
        return;
      }
      ...
      bool isHorizontal = (CGAL::abs(lineDirection.y()) < ft_epsilon && CGAL::abs(lineDirection.x()) > ft_epsilon);
      bool isVertical = (CGAL::abs(lineDirection.x()) < ft_epsilon && CGAL::abs(lineDirection.y()) > ft_epsilon);
      ...
      if (isHorizontal) {
        try {
          ...
          if (m_endPoint->getCGALPosition().y() != constrainedPos.y()) {
            m_endPoint->setCGALPosition(Point_2(m_endPoint->getCGALPosition().x(), constrainedPos.y()));
          }
          updateHostedPoints();
          update();
          return;
        ...
        try {
          ...
          if (m_endPoint->getCGALPosition().x() != constrainedPos.x()) {
            m_endPoint->setCGALPosition(Point_2(constrainedPos.x(), m_endPoint->getCGALPosition().y()));
          }
          updateHostedPoints();
          update();
          return;
    ...
    try {
      ...
      std::cerr << "Line::moveEndpointToStart: Error in default handling: " << e.what() << std::endl;
    }
  } catch (const std::exception& e) {
      ...
      if (isHorizontal) {
        try {
          ...
          if (m_startPoint->getCGALPosition().y() != constrainedPos.y()) {
            m_startPoint->setCGALPosition(Point_2(m_startPoint->getCGALPosition().x(), constrainedPos.y()));
          }
          updateHostedPoints();
          update();
          return;
        ...
        try {
          ...
          if (m_startPoint->getCGALPosition().x() != constrainedPos.x()) {
            m_startPoint->setCGALPosition(Point_2(constrainedPos.x(), m_startPoint->getCGALPosition().y()));
          }
          updateHostedPoints();
          update();
          return;
...
std::vector<Segment_2> Line::getBoundarySegments() const { return getEdges(); }
...
void Line::setAsParallelLine(std::shared_ptr<GeometricObject> refObj, int edgeIndex, const Vector_2& referenceDirection) {
  try {
    ...
    if (refObj) {
      ...
      if (Constants::DEBUG_CONSTRAINTS) {
        std::cout << "Line " << getID() << " setAsParallelLine to Object Type " << static_cast<int>(refObj->getType()) << ". Index: " << edgeIndex
                  << std::endl;
      }
...
void Line::setAsPerpendicularLine(std::shared_ptr<GeometricObject> refObj, int edgeIndex, const Vector_2& referenceDirection) {
  try {
    // 1. Clear from old reference if it exists
    if (auto old_ref_sp = m_constraintRefObject.lock()) {
    ...
    if (refObj) {
      ...
      if (Constants::DEBUG_CONSTRAINTS) {
        std::cout << "Line " << getID() << " setAsPerpendicularLine to Object Type " << static_cast<int>(refObj->getType())
                  << ". Index: " << edgeIndex << std::endl;
      }
...
bool Line::maintainConstraints() {
  ...
  if (localDebug) {
    std::cout << "Line::maintainConstraints: ENTERED for Line " << this << " (isParallel: " << m_isParallelLine
  ...
  try {
      ...
      if (ref_obj_sp->isValid()) {
        try {
          ...
          if (has_valid_reference_for_constraint) {
            ...
            if (localDebug) {
              CGALSafeUtils::debug_cgal_vector(actual_ref_dir_for_constraint, "  CURRENT Reference Direction", "MaintainConstraints");
            }
          }
    ...
    if (!has_valid_reference_for_constraint) {
      if (m_constraintDirection != Vector_2(0, 0)) {
        ...
        has_valid_reference_for_constraint = true;
        if (localDebug) CGALSafeUtils::debug_cgal_vector(actual_ref_dir_for_constraint, "  Axis Constraint Direction", "MaintainConstraints");
      } else {
    ...
    if (actual_ref_dir_for_constraint.squared_length() < Kernel::FT(Constants::CGAL_EPSILON_SQUARED)) {
      if (localDebug) {
    ...
    try {
      ...
      if (m_externallyMovedEndpoint == m_startPoint.get()) {
        ...
        if (localDebug) std::cout << "  Anchor: StartPoint (externally moved). Adjusting: EndPoint." << std::endl;
        ...
        if (localDebug) std::cout << "  Anchor: EndPoint (externally moved). Adjusting: StartPoint." << std::endl;
      } else {
        // Default: anchor on start, adjust end
        authoritative_anchor_pos = m_startPoint->getCGALPosition();
        point_to_be_adjusted_sp = m_endPoint;
        if (localDebug) std::cout << "  Anchor: StartPoint (default). Adjusting: EndPoint." << std::endl;
      }
      ...
      if (!CGAL::is_finite(authoritative_anchor_pos.x()) || !CGAL::is_finite(authoritative_anchor_pos.y())) {
      ...
      Kernel::FT sq_distance = CGAL::squared_distance(authoritative_anchor_pos, current_adjusted_pos);
      ...
      if (m_isParallelLine) {
        unit_target_dir = CGALSafeUtils::normalize_vector_robust(actual_ref_dir_for_constraint, "MaintainConstraints_ParallelDir");
      } else {  // m_isPerpendicularLine
        Vector_2 perpDir(-actual_ref_dir_for_constraint.y(), actual_ref_dir_for_constraint.x());
        unit_target_dir = CGALSafeUtils::normalize_vector_robust(perpDir, "MaintainConstraints_PerpDir");
      }
      ...
      Kernel::FT dot_product = current_vector.x() * unit_target_dir.x() + current_vector.y() * unit_target_dir.y();
      ...
      if (CGAL::to_double(distSq) < EPSILON_THRESHOLD) {
        ...
        if (localDebug) {
          std::cout << "  EPSILON GUARD: Position unchanged (distSq=" << CGAL::to_double(distSq) << " < " << EPSILON_THRESHOLD
      ...
      if (localDebug) {
        CGALSafeUtils::debug_cgal_point(current_adjusted_pos, "  Old adjusted pos", "MaintainConstraints");
        CGALSafeUtils::debug_cgal_point(new_adjusted_pos, "  New adjusted pos", "MaintainConstraints");
...
void Line::removeConstraintObserver(std::shared_ptr<Line> observer) {
  ...
  m_constraintObservers.erase(std::remove_if(m_constraintObservers.begin(), m_constraintObservers.end(),
...
void Line::attemptDataRepair() {
  ...
  try {
    ...
    try {
      ...
      std::cerr << "Line::attemptDataRepair: Error accessing endpoint positions: " << e.what() << std::endl;
      return;
    }
    ...
    if (startPos == endPos) {
      ...
      try {
        ...
        std::cerr << "Line::attemptDataRepair: Failed to fix coincident endpoints: " << e.what() << std::endl;
      }
    }
    ...
    try {
      ...
      std::cerr << "Line::attemptDataRepair: Failed to fix line equation: " << e.what() << std::endl;
    }
  } catch (const std::exception& e) {
...
void Line::clearConstraintReference(std::shared_ptr<Line> lineBeingDestroyed) {
  try {
    ...
    std::cout << "Line::clearConstraintReference: Clearing reference to Line " << lineBeingDestroyed.get() << " from Line " << this << std::endl;
    ...
    auto it =
        std::remove_if(m_constraintObservers.begin(), m_constraintObservers.end(), [&lineBeingDestroyed](const std::weak_ptr<Line>& weak_observer) {
...
// Tangent method implementations
bool Line::areTangentsShown() const {
  return m_showTangents;
}

void Line::setShowTangents(bool show) {
  m_showTangents = show;
}

float Line::getTangentLength() const {
  return m_tangentLength;
}

void Line::setTangentLength(float length) {
  m_tangentLength = length;
}

const sf::Color& Line::getTangentColor() const {
  return m_tangentColor;
}

void Line::setTangentColor(const sf::Color& color) {
  m_tangentColor = color;
}

const sf::Color& Line::getTangentOutlineColor() const {
  return m_tangentOutlineColor;
}

void Line::setTangentOutlineColor(const sf::Color& color) {
  m_tangentOutlineColor = color;
}

float Line::getTangentOutlineThickness() const {
  return m_tangentOutlineThickness;
}

void Line::setTangentOutlineThickness(float thickness) {
  m_tangentOutlineThickness = thickness;
}
